...
class DoneDetailService
{
    ...
    private const PER_PAGE  = 1000; // kalau API membatasi lebih kecil, total_pages dari API tetap dipakai
    private const MAX_PAGES = 60;   // batas pengaman per emiten per hari
    ...
    private function fetchPage(string $code, string $date, int $page): array
    {
        ...
        if (!$res->ok()) {
            throw new RuntimeException("API done-detail membalas HTTP {$res->status()}");
        }
    ...
    {
        return ['code' => $code, 'date' => $date, 'page' => $page, 'per_page' => self::PER_PAGE];
    }
